...
public class PR_도넛과막대그래프 {
	...
	//     static int[] parent;
	//     static List<List<Integer>> bj.graph;
	...
	//         List<Integer> list= bj.graph.get(center);
	...
	//         for(int start: list){
	//             //{2.8.11}
	//             List<Integer> li= bj.graph.get(start);
	...
	// 		List<Integer> centralConnections= bj.graph.get(answer[0]);
	//
	// 		//시작점 처리
	...
	//
	// 		//인접 노드 탐색
	// 		for(int next: bj.graph.get(node)){
	// 			if(!visited[next]){
	// 				//연결 정보 기록
	// 				union(node,next);
